...
echo "Grab the latest tar.gz"
       zenity --info --title="choose a version" --text="You are going to download the version ${VER}, do you want to download another one?" --no-wrap
...
wget https://github.com/PowerShell/PowerShell/releases/download/v${VER}/powershell-${VER}-linux-arm32.tar.gz || echo "Error, version not found" && sleep 3 && exit 1
