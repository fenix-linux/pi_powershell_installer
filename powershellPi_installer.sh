#!/bin/bash
#Bauh installer V1.1 for Debian PI
...
flatpak () {
    ...
    echo "A reboot is required so that flatpak packages can be installed properly."
     fi
    fi
}
alternativeapps () {
    ...
    #Snap Store + Snapd
    SNAPCHECKF=/usr/share/applications/snap/snap-store_snap-store.desktop
    SNAPCHECKD=/snap/snap-store/
    if  [ -d "$SNAPCHECKD" ] || [ -f "$SNAPCHECKF" ]  ; then
...
-You can uninstall Bauh, the Snap Store, Synaptic and Gnome-Software launching this again.
-This scipt is made to reinstall the above mentioned applications that were in FenixPI 0.9 Beta on FenixPI 0.9.1 Beta, however you can use it on any other Debian based distribution.
-Bauh description: 
...
if zenity --text-info --title="Bauh Installer V1.1   Debian* PI" --filename="$tfile"
 then
    ...
    alternativeapps
    #FenixPI 
    sudo cp $HOME/.local/bin/bauh /usr/bin &>/dev/null
    sudo rm /usr/bin/fenixexpo &>/dev/null
     if /usr/bin/fenixver | grep -q 'fenix v0.9 beta1'; then
            if zenity --question --width=400 --title="FenixPI 0.9+ detected." --text="Do you want to install zsh-syntax-highlighting, ''autosuggestions and repair | tune powerlevel10k? (ZSH is a command interpreter and Powerlevel10k is a theme for the prompt)."
            then
                echo export NO_AT_BRIDGE=1 >> /home/fenix/.zshrc
                sudo apt-get install zsh-syntax-highlighting
                sudo apt-get install zsh-autosuggestions
                sudo mkdir /usr/share/zsh-theme-powerlevel10k &>/dev/null
                sudo cp /home/fenix/powerlevel10k/powerlevel10k.zsh-theme /usr/share/zsh-theme-powerlevel10k
                sudo rm -f /home/no10/Desktop/'Add - Remove Software notema.desktop' &>/dev/null
                if zenity --question --title="ZSH-Git" --text="Do you want to disable the git plugin?"
                then
                sudo git config oh-my-zsh.hide-info 1
                fi
                zenity --info  --width=400 \
                --text="You can customize Powerlevel10k in the future with the p10k configure command."
            fi
        else 
        echo "This distro is not FenixPI V0.9 or higher, visit fenixlinux.com to discover our Raspberry distros."
     fi
    #Support
